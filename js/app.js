...
async function loadEmpresas(){
 ...
 if(!v||!Array.isArray(v)||!v.length){
   ...
   v=[];
   for(const s of [{code:"CF",name:"Cabo Frio"},{code:"AC",name:"Arraial do Cabo"}]){
 ...
 if(!(await metaGet("mig_nome_rede"))){
   ...
   for(const e of EMPRESAS){
     if(e.name==="Cabo Frio"){e.name="Cabo Frio · Super Fricarnes";mudou=true;}
     if(e.name==="Arraial do Cabo"){e.name="Arraial do Cabo · Super Fricarnes";mudou=true;}
   }
...
function renderRtInfo(){
 ...
 if(el)el.textContent="👩‍⚕️ "+(RT_INFO||"Letícia Oliveira (Nutricionista de Produção – RT) · CRN: toque p/ preencher")+"  ✎";
}
async function editarRtInfo(){
 const v=prompt("Informações da Responsável Técnica (aparecem na capa):",RT_INFO||"Letícia Oliveira (Nutricionista de Produção – RT) · CRN: ");
 if(v===null)return;
...
const PENDENCIAS_INICIAIS=[
 ...
 "Colar o arquivo interpretador.py para deixar as urgências iguais às do bot (no Lenovo)",
 "Cadastrar as áreas de Cabo Frio na aba NP · Gestão de NC (botão 🗂 Áreas)",
...
async function loadExecutores(){
 ...
 if(!v||!Array.isArray(v)||!v.length){
   v=[{nome:"Sr. João",funcao:"Manutenção"},{nome:"Matheus",funcao:"Elétrica"}];
...
function updateSubtitle(t){
  ...
  if(h1){
    ...
    /* SÓ edita no modo edição — antes ficava sempre editável e ela renomeou a aba
       sem querer só de clicar (aconteceu em 19/07: "Quadro Geral" virou "Arraial do Cabo") */
    if(aba&&MODO_EDICAO){
