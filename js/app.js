...
function metaNome(k){
  const base=String(k).replace(/Mod$/,"");
  return META_NOME[base]||(base.startsWith("areas_")?META_NOME.areas:null);
}
function histRotulo(p){
  ...
  if(metas.length===p.mudancas.length&&metas.length){
    const nomes=[...new Set(metas.map(m=>metaNome(m.chave)).filter(Boolean))];
...
const APP_DATA="26/08/2026 · 00:13";
