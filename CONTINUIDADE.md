...
- **Rotina permanente**: Routine "Sentinela GitHub — banco-demandas" (trig_01A9gex6YCHPYaDx3nX5YFPp), toda segunda ~9h (Brasília), abre sessão nova e sozinha: checa Actions/issues/PRs, roda node --check em todos os js, carrega o site headless (Playwright) caçando erros de runtime, confere expirações de token, corrige o que achar, faz push em branch claude/* e avisa Lê por push/e-mail. Cadastrada como AUT-0014 (v1.1) no Cadastro de Automações (Notion). A sentinela também lembra do `gh auth login` quando a credencial do GitHub CLI do PC expirar (aconteceu em 19/07/2026; Lê renovou no PowerShell no mesmo dia).
